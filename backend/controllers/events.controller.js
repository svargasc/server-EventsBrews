...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    cb(null, "eventImage-" + uniqueSuffix + "." + file.originalname.split(".")[1]);
  },
});
...
export const updateEventImage = async (req, res) => {
  try {
    ...
    const [existingEvent] = await pool.query("SELECT * FROM events WHERE id = ?", [eventId]);
    ...
    uploadMiddleware(req, res, async (err) => {
      ...
      if (req.file) {
        ...
        await pool.query("UPDATE event_images SET image_url = ? WHERE id = ?", [imageUrl, eventId]);
